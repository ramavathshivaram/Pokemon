import { createCards, pokemonTypes, displayUers } from "./collectionOfData.js";
...
export async function displayAllEvolutions(evolutionLines) {
  ...
  evolutionLines.forEach((evolutionLine) => {
    ...
    evolutionLineContainer.dataset.evolutionLine =
      JSON.stringify(evolutionLine);
  ...
  if (!typeData) {
    ...
    <button id="closeTypeDetails">X</button>
      ...
      <p><strong>Strengths:</strong> ${
        typeData.strengths.join(", ") || "None"
      }</p>
      <p><strong>Weaknesses:</strong> ${
        typeData.weaknesses.join(", ") || "None"
      }</p>
      <p><strong>Immunities:</strong> ${
        typeData.immunities.join(", ") || "None"
      }</p>
      <p><strong>Resistances:</strong> ${
        typeData.resistances.join(", ") || "None"
      }</p>
      <p><strong>Notable Pokémon:</strong> ${
        typeData.notablePokemon.join(", ") || "None"
      }</p>
...
export function currentUserLogin(currentUser) {
  ...
  // Format game history
  const gameHistoryHTML =
    safeUser.gameHistory.length > 0
      ? safeUser.gameHistory
          .map(
            (game) => `
            <li>
  ...
  const favoritesHTML =
    safeUser.favaratePokeom.length > 0
      ? safeUser.favaratePokeom
          .map(
            (pokemon) => `
  ...
  const loginHistoryHTML =
    safeUser.loginHistory.length > 0
  ...
  const securityQuestionsHTML =
    safeUser.securityQuestions.length > 0
      ? safeUser.securityQuestions
          .map(
            (q, i) => `
            <li>
                ...
                <section class="profile-section">
                    ...
                    <div class="preferences">
                        ...
                        <p><strong>Notifications:</strong> ${
                          safeUser.preferences.notifications ? "On" : "Off"
                        }</p>
  ...
  document.getElementById("editPreferences")?.addEventListener("click", () => {
    ...
    safeUser.preferences.theme =
      safeUser.preferences.theme === "light" ? "dark" : "light";
...
function showAddFavoriteModal(safeUser, container) {
  ...
  // Close button
  addFavoriteModal
    .querySelector("#closeAddFavorite")
...
export function pokemonCountDisplay(count) {
  document.getElementById("pokemonCount").innerHTML = `<span>${count}</span>`;
}
